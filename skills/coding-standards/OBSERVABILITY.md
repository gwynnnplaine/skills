...
**Structured Tracing** — Correlated telemetry across requests, jobs, modules, adapters, and external calls.
...
- Secrets never enter errors, logs, traces, metrics, snapshots, test snapshots, or serialized diagnostics.
...
An observability pass is complete when every touched adapter, error translator, framework handler, or background task has been checked for:
...
- Use a small local `Redacted<T>` wrapper (e.g. in `prelude.ts`) when the project lacks one; if the repo already has a redaction mechanism, use it.
...
Avoid carrying raw secret strings through modules:
...
Before adding an adapter, error translator, framework handler, or background task, inspect how the repo currently reports:
...
- **"Instrumentation belongs in domain logic."** Domain logic returns decisions; adapters and the shell observe them.
