...
Permitted `any` is rare, targeted, and justified with your linter's disable directive:

```ts
// eslint-disable-next-line no-explicit-any -- SAFETY: This helper preserves arbitrary function parameters; TypeScript cannot express the variadic constraint without any.
...
Use the repository's established formatter, linter, type-checker, and test runner; do not weaken them to admit changed code. Keep tool defaults unless a correctness requirement or demonstrated false positive justifies an override; do not enable whole lint categories merely to enforce aesthetic preferences.

Preserve default correctness checks for assertions, async loops, parameter mutation, caught-error preservation, JSDoc structure, accumulating spread, barrels, unsafe type operations, floating promises, invalid thrown values, nullish defaults, switch exhaustiveness, unknown catch callbacks, module mocks, and method spies.

Use strict compiler settings including exact optional-property behavior, unchecked-index protection, implicit-return checks, fallthrough protection, explicit override checks, and full strictness. Warnings and unused suppressions are not tolerated. Preserve established toolchains unless migration is explicitly in scope.
