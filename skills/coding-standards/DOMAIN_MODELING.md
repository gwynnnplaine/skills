...
- An invariant asserted in a comment, name, or doc is enforced by a type, constructor, or transition. A comment is a hope; a type is a guarantee.
...
When your runtime schema library can express the brand, use the library's branding/refinement rather than a handwritten cast.
...
If absence is a real domain concept, name it clearly: `maybeFindById`, `lookupOptional`, or `exists`.
...
`Partial<T>` is acceptable when partiality is the domain concept, such as patch semantics owned by an adapter or a named patch command.
...
A comment asserting an invariant is the signal that the invariant exists but is not modeled.

Avoid:

```ts
// data is set once loading is false — trust me
type FetchState<T> = { isLoading: boolean; data?: T; error?: Error };
```
...
The comment disappears because the compiler now owns the claim. When you catch yourself writing "can't happen here", "always set when", or "backs the type" — stop and model it.
...
- Asserting an invariant in a comment ("can't happen here", "always set when X") instead of a type that makes it true.
