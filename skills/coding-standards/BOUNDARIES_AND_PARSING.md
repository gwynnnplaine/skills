...
**Unknown Boundary Input**, **External Adapter Module**, **Persistence Boundary Parser**, **Protocol Projection**, and **Persistence Projection** live in [`VOCABULARY.md`](VOCABULARY.md).

## Non-negotiables

- Unknown boundary input stays `unknown` or boundary DTO-shaped until parsed.
- External input is parsed in the External Adapter Module, handler, or composition entrypoint before service/core code sees it; correct-by-construction values are passed inward.
- Decoded JSON, response bodies, env values, queue messages, and storage JSON are not cast into domain/service types.
...
- Use the repository's established runtime schema library when it satisfies the contract; if no schema convention exists, pick one and stay consistent.
...
```ts
const CreateUserBody = strictObject({
...
```ts
type InvoiceRow = /* your ORM's inferred row type */;
...
Values crossing process, runtime, RPC, queue, or structured-clone boundaries must be serializable for that transport.
...
- Passing ORM rows directly to Service Modules.
