...
**Caller-Owned Cancellation Lifetime** and **Detached Work** live in [`VOCABULARY.md`](VOCABULARY.md). Local terms:
...
- Independent async work starts concurrently unless ordering/backpressure/rate limit/transaction/external contract requires serialization.
...
Async work that waits on I/O, timers, retries, queues, subprocesses, resource acquisition, or long computation should accept caller-owned cancellation when the runtime supports it.
...
Detached work goes through the runtime/project mechanism, which must own rejection handling and observability:

```ts
runDetached(sendWelcomeEmail(user), { signal, logger });
// or ctx.waitUntil(sendWelcomeEmail(user)) if the runtime provides it
```
